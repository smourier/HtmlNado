...
public static class Conversions
{
    ...
    public static string ToHexa(this byte[] bytes, int offset, int count = 0)
    {
        if (bytes == null)
            return null;
    ...
    public static string? SafeSubstring(this string? thisString, int startIndex, int length = int.MaxValue)
    {
        ...
        if (startIndex < 0 || startIndex >= thisString.Length)
            return null;
    ...
    public static bool TryChangeType(object input, Type conversionType, IFormatProvider provider, out object value)
    {
        ...
        {
            var nullableType = conversionType.GenericTypeArguments[0];
            if (input == null)
            {
                value = null;
                return true;
